...
CHAR.prototype.validate = function(value) {
  var method = this._binary ? Validator.isByteLength : Validator.isLength;

  if (!method.call(value, this._length)) {
    throw new sequelizeErrors.ValidationError(util.format('%j doesn\'t have a valid length', value));
  }
...
INTEGER.prototype.validate = function(value) {
  if (!Validator.isInt(value)) {
    throw new sequelizeErrors.ValidationError(util.format('%j is not a valid int', value));
  }
...
DATE.prototype.validate = function(value) {
  if (!Validator.isDate(value)) {
    throw new sequelizeErrors.ValidationError('Invalid date');
  }
...
ARRAY.validate = function(value) {
  if (!util.isArray(value)) {
    throw new sequelizeErrors.ValidationError(util.format('%j is not an array', value));
  }
